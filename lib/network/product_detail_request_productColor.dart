...
class requestProductDetailColor {
  static String url =
      "http://192.168.1.220:9000/api/getProductDetailColor/$productSKU";

  static List<api_product_details> parseProductDetailColor(
      String responseBody) {
  ...
  static Future<List<api_product_details>> fetchProductDetailColor(
      {int page = 1}) async {
    final response = await http.get(Uri.parse('$url'));
    if (response.statusCode == 200) {
      return compute(parseProductDetailColor, response.body);
